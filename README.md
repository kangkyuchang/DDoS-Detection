...
| 단계 | 기법 | 상세 내용 |
|:---|:---|:---|
| **결측치 처리** | **평균값 대체** | 데이터 이상치 영향 최소화를 위해 해당 Feature의 **평균값**을 이용하여 결측치 처리 |
...
-  **기술적 원인 및 보완 분석**:
    ...
    2.  **특징 누락 가능성**: SYN Flooding 공격은 TCP 3-way-handshake 과정 중 서버의 ACK 응답에 대한 클라이언트의 최종 ACK를 보내지 않아 서버의 연결 리소스를 고갈시키는 공격입니다.
     ACK Flag Count는 이러한 불완전한 연결 시도의 핵심적인 특징을 나타냅니다. 특징 추출 과정에서 해당 특징이 최종 21개 Feature에 포함되지 않아 AutoEncoder가 SYN Flooding의 핵심적인 비정상 패턴을 학습할 기회를 잃었을 가능성이 있습니다.
